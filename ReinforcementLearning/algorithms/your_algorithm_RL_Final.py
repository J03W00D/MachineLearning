...
# Enhanced CMA-ES Implementation with Antithetic Sampling and Adaptive Clamping
    if not hasattr(your_optimization_alg, "initialized"): #Check if algorithm is initalised (set-up step)
        ...
        your_optimization_alg.pc = torch.zeros(n_params)  # Covariance learning...
        ...
        # Selection weights (log-weighted as favoured better solutions more favourably)
        weights = torch.tensor([np.log(your_optimization_alg.mu + 0.5) - np.log(i + 1) 
                               for i in range(your_optimization_alg.mu)])
        your_optimization_alg.weights = weights / weights.sum() #Normalise weights so they sum to 1
        ...
        # Best solution tracking for restarts..
    ...
    for iteration in tqdm(range(num_iterations)):
        ...
        for x in population_params:
            ...
            # Track absolute best for restarts REDUNDANT as removed!
        ...
        # Update pc (as explained earlier) (for covariance learning)
