...
# 1. Enhanced CMA-ES with Strategic Improvements
    if not hasattr(your_optimization_alg, "initialized"):
        ...
        # CMA-ES Hyperparameters - Optimized for 5-minute budget
        ...
        your_optimization_alg.sigma_vec = torch.ones(n_params) * 0.5 #0.2 from 0.5
        ...
        your_optimization_alg.damps = 2.0 #2 from 1
        ...
        # Learning rates
        your_optimization_alg.c_cov = 0.08  # Faster covariance learning #0.08 from 0.25
    ...
    for iteration in tqdm(range(num_iterations)):
        ...
        for x in population_params:
            ...
            while not done:
                # Standardize state for network input
        ...
        # C. ENHANCED SELECTION: Fitness shaping to reduce outlier influence
       # indices = np.argsort(population_rewards)[::-1]
        #elite_indices = indices[:your_optimization_alg.mu]
        
        # Fitness shaping: clip extreme values
       # elite_rewards = np.array([population_rewards[i] for i in elite_indices])
       # median_elite = np.median(elite_rewards)
       # shaped_rewards = np.clip(elite_rewards, median_elite * 0.6, median_elite * 1.4)
        
        # Update mean (weighted recombination)
        old_mean = your_optimization_alg.mean.clone()
        #your_optimization_alg.mean = torch.zeros_like(your_optimization_alg.mean)
        #for i, idx in enumerate(elite_indices):
         #   your_optimization_alg.mean += your_optimization_alg.weights[i] * population_params[idx]
        ...
        # E. ENHANCED COVARIANCE: Rank-mu + Rank-one updates
       # weighted_variance = torch.zeros_like(your_optimization_alg.sigma_vec)
        
        # Rank-mu: learn from all elites
       # for i, idx in enumerate(elite_indices):
        #    delta = population_deltas[idx]
         #   weighted_variance += your_optimization_alg.weights[i] * (delta ** 2)
        
        # Rank-one: learn from cumulation path
        #rank_one_update = your_optimization_alg.pc ** 2
        
        # Combined update
        #your_optimization_alg.sigma_vec = ((1 - your_optimization_alg.c_cov - your_optimization_alg.c1) * 
         #                                  your_optimization_alg.sigma_vec + 
          #                                 your_optimization_alg.c_cov * torch.sqrt(weighted_variance + 1e-8) +
           #                                your_optimization_alg.c1 * torch.sqrt(rank_one_update + 1e-8))
        ...
        # G. SMART RESTART: Detect stagnation and restart with best
       # your_optimization_alg.no_improvement_count += 1
       # improvement_threshold = 300  # Minimum improvement to reset counter
        
       # if iteration > 10:
        #    recent_best = max(population_rewards)
         #   if recent_best > your_optimization_alg.best_ever_reward - improvement_threshold:
          #      your_optimization_alg.no_improvement_count = 0
        
        # Restart if stuck for too long
       # if your_optimization_alg.no_improvement_count >= 20 and iteration < num_iterations - 10:
        #    print(f"\n[Restart] Stagnation detected at iteration {iteration}. Restarting from best.")
            
            # Reset to best-ever with increased exploration
         #   your_optimization_alg.mean = your_optimization_alg.best_ever_params.clone()
          #  your_optimization_alg.sigma_vec = torch.ones(len(your_optimization_alg.mean)) * 0.6
           # your_optimization_alg.ps = torch.zeros_like(your_optimization_alg.ps)
           # your_optimization_alg.pc = torch.zeros_like(your_optimization_alg.pc)
           # your_optimization_alg.no_improvement_count = 0
        
        # H. PERIODIC EXPLORATION INJECTION: Re-diversify late in training
        #if iteration > num_iterations * 0.6 and iteration % 10 == 0:
            # Small exploration boost every 10 iterations in final 40%
          #  noise_boost = torch.randn_like(your_optimization_alg.sigma_vec) * 0.1
          #  your_optimization_alg.sigma_vec = torch.abs(your_optimization_alg.sigma_vec + noise_boost)
          #  your_optimization_alg.sigma_vec = torch.clamp(your_optimization_alg.sigma_vec, 0.01, 1.5)
        # Periodic exploration boost
        #if iteration > 0 and iteration % 20 == 0:  # Every 20 iterations
         #   your_optimization_alg.sigma = min(your_optimization_alg.sigma * 1.5, 0.3)
        # -----------------------------------------------------------------------------------
